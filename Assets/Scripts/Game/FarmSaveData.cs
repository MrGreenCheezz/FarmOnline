...
namespace Farm.Game
{
    ...
    public sealed class PlotSave
    {
        ...
        /// <summary>Накормлена ли скотина в текущем цикле. Свой флаг: уход у неё свой.</summary>
        public bool Fed;
    ...
    public sealed class FarmerSave
    {
        ...
        /// <summary>
        /// Разрешено ли жителю тратить золото фермы на грядки и живность.
        /// <para>
        /// Поля не было до 07.08.2026, и его отсутствие читается как «разрешено» — именно так
        /// вели себя все партии до переключателя. Ставить умолчанием запрет значило бы молча
        /// выключить поведение, к которому игрок привык.
        /// </para>
        /// </summary>
        public bool MayRestock = true;
    ...
    public sealed class FarmSaveData
    {
        ...
        // Счётчики онлайна (этап 8) — под вехи социальной половины игры.
        // У старых сейвов полей нет — нули, снисходительно.
        public int FriendsSeen;
        public int HelpGiven;
        public int GiftsSent;
        public int MarketLots;
        ...
        /// <summary>
        /// Корм для скотины: мешки и момент последнего начисления — как у воды, и по той же
        /// причине (копится в постройке по реальным часам). Ноль в <see cref="FeedFilledUnix"/>
        /// у старых партий читается как «начать копить с этой минуты», а не как долг.
        /// </summary>
        public int FeedCharges;
        public double FeedFilledUnix;
        ...
        /// <summary>
        /// Заказы, за которые уже заплачено. Сами заказы хранятся ниже (<see cref="OrdersBoard"/> —
        /// доска застыла при генерации); здесь — ровно отметки сдачи. Чтение отсеивает записи,
        /// не относящиеся ни к текущему окну, ни к доске из сейва.
        /// </summary>
        public string[] FilledOrders = Array.Empty<string>();

        /// <summary>
        /// Доска заказов, застывшая при генерации (залог стабильности: живой пул больше
        /// не перекатывает несданное — см. FarmOrders). Окно + сами заказы; старые сейвы
        /// без этих полей просто перегенерируют доску — снисходительно.
        /// </summary>
        public long OrdersWindow = long.MinValue;
        public OrderSave[] OrdersBoard = Array.Empty<OrderSave>();
