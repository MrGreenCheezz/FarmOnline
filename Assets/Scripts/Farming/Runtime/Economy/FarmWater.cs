...
namespace Farm.Farming
{
    ...
    public static class FarmWater
    {
        ...
        public static bool Pour(Growable plot)
        {
            ...
            if (!plot.CanWater)
            {
                // Категорию называем вслух: с постоянным инструментом в руке ведро по корове
                // стало штатной ошибкой, а «поливать нечего» про явно растущую корову читается
                // как поломка, а не как «взял не то» (симметрично отказу корма).
                Refuse(plot.Category != ResourceCategory.Crop
                        ? "ведро — для грядок"
                        : plot.Watered ? "уже полито" : "поливать нечего", at);
                return false;
            }
