...
namespace Farm.UI
{
    ...
    public sealed class MergeHint : MonoBehaviour
    {
        ...
        // Инструмент назван прямо: с 07.08.2026 рука грядку не поднимает, и урок
        // «потяни одну на другую» без «Переноса» звал бы к жесту, который молчит.
        private const string Lesson = "возьми внизу «Перенос» и потяни грядку на такую же — уровни сложатся";
        ...
        private void LateUpdate()
        {
            ...
            // Гостю урок не показывается: перетаскивание в гостях отключено, а «потяни одну
            // на другую» про чужие грядки — приглашение к жесту, который молча не сработает
            // (TotalMerges тут вдобавок хозяйский, у новичка урок гостю гарантирован).
            if (_done || GuestMode.IsGuest || FarmProgress.TotalMerges > 0) { Hide(); return; }
